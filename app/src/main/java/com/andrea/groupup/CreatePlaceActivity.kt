...
class CreatePlaceActivity : AppCompatActivity() {
    ...
    private fun createPlace(){
        if(title.text.isNotEmpty() && address.text.isNotEmpty() && description.text.isNotEmpty() && rating.text.isNotEmpty()){
            place = LocalPlace(0, title.text.toString(), location.latitude.toString(), location.longitude.toString(), address.text.toString(), null, Date(), user.id, user.UserGroup.GroupId, ArrayList<Photo>(), rating.text.toString().toDouble(), ArrayList<Tag>(), 0.0, rating.text.toString().toInt(), 0,  null)
